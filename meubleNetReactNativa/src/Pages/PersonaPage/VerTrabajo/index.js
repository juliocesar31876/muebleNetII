...
const VerTrabajo = (props) => {
    ...
    const popupPersona = () => {
        const selecEmpleado = (objEmpleado) => {
            ...
            props.getPersonaTrabajo(props.state.socketReducer.socket,
                {
        ...
        <View style={{
            ...
            <View style={{ width: "90%", alignItems: 'center', justifyContent: 'center', flexDirection: 'row', marginTop: 20, }}>
                ...
                <TouchableOpacity
                    ...
                    {state.persona.data ? (
                        <Text style={{ color: "#fff", fontSize: 10, textAlign: 'center' }}> {state.persona.value} </Text>
                    ) : (
                            <Svg name={"persona"}
                                style={{
                                    width: 35,
            ...
            <View style={{ width: "90%", alignItems: 'center', justifyContent: 'center', flexDirection: 'row', marginTop: 20, }}>
                <Text style={{ color: "#fff", flex: 0.7, margin: 5, }}>Mes trabajo:</Text>
                <TouchableOpacity
            ...
            <ScrollView style={{ flex: 1, width: "100%",marginTop: 20, }}>
                <View style={{ width: "100%", margin: 2, flex: 1, alignItems: 'center', }}>
                    {Object.keys(props.state.personaReducer.dataTrabajoPersona).map((key) => {
                        var obj = props.state.personaReducer.dataTrabajoPersona[key]
                        var fecha = moment(obj.fecha_on, "YYYY-MM-DD").format("DD/MM/YYYY");
                        return (
                            <View style={{ width: "90%", flexDirection: 'row', alignItems: 'center', borderBottomWidth: 2, borderColor: "#666", margin: 5, borderRadius: 10, }}>
                                <Text style={{ color: "#fff", fontSize: 13, flex: 1, margin: 5, }}> Producto: {obj.nombre} </Text>
                                <View style={{flex: 0.9,alignItems: 'center',}}>
                                    <Text style={{ color: "#fff", fontSize: 10, flex: 1, margin: 5, }}> Fecha :   {fecha} </Text>
                                    <Text style={{ color: "#fff", fontSize: 10, flex: 1, margin: 5, }}> Cantidad :{obj.cantidad} </Text>
                                </View>
                            </View>
                        )
                    })}
...
export default connect(initStates, initActions)(VerTrabajo);
